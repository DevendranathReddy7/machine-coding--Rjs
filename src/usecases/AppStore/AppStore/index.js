...
class AppStore extends Component {
  state = { searchValue: "", tabVal: "SOCIAL" };

  tabHandler = (id) => {
    this.setState({ tabVal: id });
  };
  ...
  render() {
    const { searchValue, tabVal } = this.state;
      ...
      <center>
        ...
        <div className="search__container">
          ...
          <div className="search__img">
            {" "}
            <CiSearch color="black" size="25px" />
          </div>
        </div>
        ...
        <ul className="app__container">
          {appsList
            .filter(
              (cat) =>
                cat.category === tabVal &&
                cat.appName.toLowerCase().includes(searchValue.toLowerCase())
            )
            .map((app) => (
              <AppItem key={app.appId} currentApp={app} />
            ))}
        </ul>
      </center>
    );
